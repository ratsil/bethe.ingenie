        ...
        abstract private class Request
        {
            ...
            public class Weather : Request
            {
                ...
                public override XmlNode this[byte nTemplateID, object oValue]
                {
                    get
                    {
                        if (0 > nTemplateID || 1 < nTemplateID)
                            throw new Exception("unknown weather request template [" + nTemplateID + "]");
						return Yandex(nTemplateID, ((string)oValue).Split(';'));
                    }
                }

                private XmlNode Yandex(byte nTemplateID, string[] aRegions)
                {
                    ...
                    string sValue;
                    XmlDocument cXmlDocument = new XmlDocument();
                    XmlNode cResult = cXmlDocument.CreateNode(XmlNodeType.Element, "result", null);
                    Region cRegion = null;
					XmlNode[] aXNDates;
					foreach (string sRegion in aRegions.Select(o => o.Trim().ToLower()))
                    {
						try
						{
                            ...
                            cXmlDocument = cResult.OwnerDocument;
                            switch (nTemplateID)
                            {
                                case 0:
                                    cXmlNode = cXmlDocument.CreateNode(XmlNodeType.Element, "item", null);
                                    cRegion.Template0Item(cXmlNode);
                                    cResult.AppendChild(cXmlNode);
                                    break;
                                case 1:
                                    if (1 > cResult.ChildNodes.Count)
                                    {
                                        cResult.AppendChild(cXmlNode = cXmlDocument.CreateNode(XmlNodeType.Element, "item", null));
                                        cXmlNode.AttributeAdd("output" , "000");
                                    }
                                    else
                                        cXmlNode = cResult.ChildNodes[cResult.ChildNodes.Count - 1];

                                    cRegion.Template1Item(cXmlNode, "POST", Period.Now);
                                    cXmlNode = cXmlDocument.CreateNode(XmlNodeType.Element, "item", null);
									cXmlNode.AttributeAdd("output", cResult.ChildNodes.Count.ToString("000"));
